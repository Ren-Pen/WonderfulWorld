...
namespace slimenano::math {
    ...
    namespace internal {
        template <Arithmetic T> class base_vector_support_2 {
          public:
            base_vector<T, 2> rotate(T angle) const {
                ...
                return base_vector<T, 2>(static_cast<T>(that[0] * cos_theta - that[1] * sin_theta),
        ...
        template <Arithmetic T> class base_vector_support_3 {
          public:
            base_vector<T, 3> cross(const base_vector<T, 3> &other) const {
                auto &that = (*static_cast<base_vector<T, 3> *>(this));

                return base_vector<T, 3>(that[1] * other[2] - that[2] * other[1],
        ...
        template <Arithmetic T, size_t N> class base_vector_properties_2 {
          public:
            ...
            const T &y() const { return (*static_cast<base_vector<T, N> *>(this))[1]; }
            T &g() { return (*static_cast<base_vector<T, N> *>(this))[1]; }
            const T &g() const { return (*static_cast<base_vector<T, N> *>(this))[1]; }
        };

        template <Arithmetic T, size_t N> class base_vector_properties_3 {
          public:
            T &z() { return (*static_cast<base_vector<T, N> *>(this))[2]; }
            const T &z() const { return (*static_cast<base_vector<T, N> *>(this))[2]; }
            T &b() { return (*static_cast<base_vector<T, N> *>(this))[2]; }
            const T &b() const { return (*static_cast<base_vector<T, N> *>(this))[2]; }
        };

        template <Arithmetic T, size_t N> class base_vector_properties_4 {
          public:
            T &w() { return (*static_cast<base_vector<T, N> *>(this))[3]; }
            const T &w() const { return (*static_cast<base_vector<T, N> *>(this))[3]; }
            T &a() { return (*static_cast<base_vector<T, N> *>(this))[3]; }
            const T &a() const { return (*static_cast<base_vector<T, N> *>(this))[3]; }
        };
    ...
    class base_vector : public internal::base_vector_support<T, N>, public internal::base_vector_properties<T, N> {

        template <Arithmetic, std::size_t> friend class base_vector;
      ...
      public:
        ...
        base_vector(const std::array<T, N> &array) { std::copy(array.begin(), array.end(), data.begin()); }

        base_vector(const base_vector &other) { std::copy(other.data.begin(), other.data.end(), data.begin()); }
        ...
        base_vector(base_vector &&other) noexcept : data(std::move(other.data)) {}
      ...
      private:
        template <typename BinaryOperation> base_vector binary_op(const base_vector &other, BinaryOperation op) const {
            base_vector result;
            std::transform(data.begin(), data.end(), other.data.begin(), result.data.begin(), op);
            return result;
        }

        template <typename BinaryOperation> base_vector &binary_self_op(const base_vector &other, BinaryOperation op) {
            std::transform(data.begin(), data.end(), other.data.begin(), data.begin(), op);
            return *this;
        }

        template <typename UnaryOperation> base_vector &unary_op(UnaryOperation op) {
            std::transform(data.begin(), data.end(), data.begin(), op);
            return *this;
        }

      public:
        base_vector operator+(const base_vector &other) const { return binary_op(other, std::plus<T>()); }

        base_vector &operator+=(const base_vector &other) { return binary_self_op(other, std::plus<T>()); }

        base_vector operator-(const base_vector &other) const { return binary_op(other, std::minus<T>()); }

        base_vector &operator-=(const base_vector &other) { return binary_self_op(other, std::minus<T>()); }

        base_vector operator*(T scalar) const {
            return binary_op(*this, [scalar](T a) {
                return a * scalar;
            });
        }

        base_vector operator*=(T scalar) const {
            return unary_op([scalar](T a) {
                return a * scalar;
            });
        }
      ...
      public:
        T dot(const base_vector &other) const {
            return std::inner_product(data.begin(), data.end(), other.data.begin(), 0);
        }

        T sqrt_length() const { return this->dot(*this); }

        T length() const { return static_cast<T>(std::sqrt(this->sqrt_length())); }
        ...
        T distance(const base_vector &other) const { return ((*this) - other).length(); }

        base_vector projection(const base_vector &other) const {
            T dot_product = this->dot(other);
            T sqrt_length = other.sqrt_length();
            base_vector result;
            std::transform(other.data.begin(), other.data.end(), result.data.begin(), [dot_product, sqrt_length](T a) {
                return static_cast<T>((dot_product / sqrt_length) * a);
            });
            return result;
        }

        base_vector translate(const base_vector &offset) const { return (*this) + offset; }

        base_vector rejection(const base_vector &other) const { return *this - this->projection(other); }
    };

    template <Arithmetic T, std::size_t N> std::ostream &operator<<(std::ostream &os, const base_vector<T, N> &vector) {
